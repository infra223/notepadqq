...
void Context::load(QXmlStreamReader& reader)
{
    ...
    m_noIndentationBasedFolding =
        Xml::attrToBool(reader.attributes().value(QStringLiteral("noIndentationBasedFolding")));
...
void Context::resolveIncludes()
{
    ...
    for (auto it = m_rules.begin(); it != m_rules.end();) {
        ...
        if (inc->definitionName().isEmpty()) { // local include
            ...
            if (!def.isValid()) {
                qWarning() << "Unable to resolve external include rule for definition" << inc->definitionName() << "in"
        ...
        if (!context) {
            qWarning() << "Unable to resolve include rule for definition" << inc->contextName() << "##"
                       << inc->definitionName() << "in" << m_def.definition().name();
            ++it;
            continue;
        }
